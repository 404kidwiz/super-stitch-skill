...
legacy_alias_path="$repo_root/.codex/skills/google-stitch-design/SKILL.md"
...
smoke_prompt="Use the super-stitch-skill skill. What is the third item under \"Read In This Order\"? Reply with only the relative file path."
...
if [ -f "$legacy_alias_path" ]; then
  pass "Legacy alias still exists -> $legacy_alias_path"
fi

gemini_enable_output="$(gemini skills enable "$skill_name" 2>&1 || true)"
if printf '%s\n' "$gemini_enable_output" | grep -Eq "already enabled|Enabled skill"; then
